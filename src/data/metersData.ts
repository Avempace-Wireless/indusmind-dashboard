/**
 * Centralized Meters Data
 * Single source of truth for all meter data across the application
 * Used by Dashboard, Power, History, and Comparison views
 */
...
function generateHourlyData(baseValue: number, variation: number): DataPoint[] {
  ...
  for (let i = 0; i < 24; i++) {
    ...
    let value = baseValue
    if (i >= 6 && i < 22) {
      value = baseValue + Math.random() * variation
    } else {
      value = baseValue * 0.6 + Math.random() * (variation * 0.4)
    }
...
export function getMetersByCategory(category: typeof CATEGORIES[number]): Meter[] {
  return METERS_DATA.filter(meter => meter.category === category)
}
...
export function getMeterById(id: string): Meter | undefined {
  return METERS_DATA.find(meter => meter.id === id)
}
