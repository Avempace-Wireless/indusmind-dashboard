...
const totalTablePages = computed(() => {
  ...
  if (activeMeterIds.value.length > 0) {
    filteredTable = comparisonTable.value.filter((row: ComparisonTableRow) => {
...
const paginatedComparisonTable = computed(() => {
  const filteredTable = activeMeterIds.value.length > 0
    ? comparisonTable.value.filter((row: ComparisonTableRow) => activeMeterIds.value.some(id => row.label.includes(getMeterName(id))))
...
function initChart() {
  ...
  if (comparisonMode.value === 'byMeters') {
    ...
    const visibleData = activeMeterIds.value.length > 0
      ? comparisonData.value.filter((d: ComparisonDataItem) => activeMeterIds.value.includes(d.meterId || d.id || ''))
    ...
    const uniquePeriods = [...new Set(comparisonData.value.map((d: ComparisonDataItem) => (d.periodLabel || d.label || d.periodId || '')))]
    const labels = uniquePeriods.sort().filter((l: string) => l !== '')
